...
namespace FA.JustBlog.Core.Repository
{
	public class PostsRepository : BaseRepository<Posts>, IPostsRepository
	{
		...
		{
			return await _DbContext.Posts.Include(it => it.Categories).Include(it => it.PostTags).ThenInclude(it => it.Tags).Where(it => it.Published == true).Take(size).ToListAsync();
		}
		...
		{
			return await _DbContext.Posts.Include(it => it.Categories).Include(it => it.PostTags).ThenInclude(it => it.Tags).Take(size).ToListAsync();
		}
	}
}